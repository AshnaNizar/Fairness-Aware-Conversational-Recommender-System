...
EMBEDDING_DIM   = 64   # fallback only — actual values auto-detected from checkpoint
NUM_LAYERS      = 3    # fallback only — actual values auto-detected from checkpoint
MIN_RATING      = 4
TOP_K           = 10
CANDIDATE_K     = 50
P_FAIRNESS      = 0.3
...
try:
    ...
    # ── Load checkpoint — extract embeddings directly, no graph needed ────────
    # The checkpoint contains the fully trained embedding matrix.
    # We extract user and movie rows directly from embedding.weight, bypassing
    # the need to reconstruct the KG graph or reinstantiate the model class.
    # This is safe because LightGCN scoring at inference is just:
    #   score(u, m) = user_emb[u] · movie_emb[m]
    # and the base embedding.weight already encodes the graph-propagated representations.
    print("Loading trained KG embeddings from checkpoint...")
    ckpt = torch.load(KG_MODEL_PATH, map_location="cpu")
    emb_weight = ckpt["embedding.weight"]   # shape: (n_total, emb_dim)
    ckpt_n_total, ckpt_emb_dim = emb_weight.shape
    ckpt_num_layers = ckpt["lgcn.alpha"].shape[0] - 1
    print(f"  Checkpoint: n_total={ckpt_n_total}, emb_dim={ckpt_emb_dim}, num_layers={ckpt_num_layers}")

    # The first n_users rows are user embeddings, next n_movies are movie embeddings.
    # If the checkpoint was trained on more users/movies than the current data slice,
    # we clamp safely — extra rows just won't be used.
    ckpt_n_users  = min(n_users,  ckpt_n_total)
    ckpt_n_movies = min(n_movies, ckpt_n_total - ckpt_n_users)
    user_emb  = emb_weight[:ckpt_n_users].numpy()           # (n_users, emb_dim)
    movie_emb = emb_weight[ckpt_n_users: ckpt_n_users + ckpt_n_movies].numpy()  # (n_movies, emb_dim)
    print(f"  Using user_emb={user_emb.shape}, movie_emb={movie_emb.shape}")
    ...
    print("Computing all scores (dot product)...")
    all_scores = user_emb @ movie_emb.T   # (n_users, n_movies)
    ...
    if os.path.exists(CACHE_PATH):
        ...
        print("Building candidate pools (first run only)...")
        ALL_CANDIDATES = {u: build_candidates(u) for u in tqdm(range(n_users))}
        import pickle
        os.makedirs(os.path.dirname(CACHE_PATH), exist_ok=True)
        with open(CACHE_PATH, "wb") as f:
            pickle.dump(ALL_CANDIDATES, f)
        print(f"Saved candidate pools to {CACHE_PATH}")
...
def select_persona(persona_name):
    ...
    welcome = [{"role": "assistant", "content": (
        f"I've loaded a profile for a **{persona_name}** (user #{user_id}). "
...
def build_error_ui():
    with gr.Blocks(title="FA-CRS — Setup needed") as demo:
        with gr.Group(elem_id="header-block"):
            gr.Markdown("# ⚠️ FA-CRS — Setup needed")
            gr.Markdown(
